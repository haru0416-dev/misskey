...
`2026.6.0` 以前は upstream `misskey-dev/misskey` のリリース履歴。このリポジトリは upstream 追従を
行わず、これらのバージョンとしてリリースしたことも無いため、`CHANGELOG.md` から分離した。
現在進行中の変更は [../../CHANGELOG.md](../../CHANGELOG.md) の `## Unreleased` に記載する。
