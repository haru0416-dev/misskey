...
### MFMパースの分岐表と2乗の解消（2026-09-25）

1文字あたりのパース時間（4種の文、各400回×7組の中央値）は、数字なしの日本語44〜66 ns に対し、数字入りの日本語3,000〜3,450 ns・英語1,390〜1,540 ns だった。数字は絵文字のキーキャップのため平文の表で止まる。さらに長さ17→1,088字で2,088→7,349 ns/字と伸びており、CPUの73%が検索構文の事前判定だった。730a8a1795で入れたこの判定は、行頭かを見る前に行末まで走査・切り出していたため、構文を始め得る文字のたびに行の残りを読み直していた。行頭でなければ先に失敗させ（元の文法の lineBegin と同じ条件）、長さによらず約1,600 ns/字にした。mention・italicAsta・italicUnderの直前文字判定も、先頭からの切り出しに正規表現をかけていたので直前1文字の判定にした（非unicodeの i フラグでは ASCII の大文字小文字だけを同一視するので等価）。

そのうえで、構文ごとに最初に消費し得る文字（RULE_STARTS）から、位置の先頭文字と行頭かどうかで候補を絞った P.alt を試す分岐表を入れた。候補は元の順序のまま、組み合わせごとに1つ共有する（数十通り）。結果は数字入りの日本語3,000〜3,450→約67 ns/字、英語1,390〜1,540→約47、MFM記法の多い文270〜284→約110、数字なしの日本語約45→約35。元の文法との差分テスト（fast-check）は通り、mentionの開始文字・searchの行頭指定・絵文字の指定を外す変異はそれぞれ17件・7件・9件で落ちた。直前文字判定は両方の文法に入るため差分テストでは見えず、既存のパーサのテストが判定位置をずらす変異を4件で落とすことを確かめた。

### 入力の大きさに対する計算量の棚卸し（2026-09-25）

DBの件数に対する伸び方は以前に測り終えているので、今回は1回の要求・活動・投稿の入力の大きさに対するCPUの伸び方を見た。候補はコード全体の読み取りで14件挙がり、上位8件を関数単位で大きさを変えて測り（両対数の傾きk）、直した。

| 箇所 | 入力 n | 変更前 | 変更後 |
| --- | --- | --- | --- |
| HTTP署名ヘッダーの解析（署名検証の前、誰でも送れる） | ヘッダー長 | k=1.97、16 KB で 260 ms | k≈0.9、38 µs |
| チャート保存時の差分の集約 | 20分間の差分数 | k=1.59、4万件で 450 ms | k=0.96、5 ms |
| アンテナのキーワード照合 | キーワード数（上限なし） | 1,600語・3,000字で 10 ms/投稿/アンテナ | 小文字化1回で 0.46 ms、行列は100×10語×200字まで |
| ハッシュタグのストリーム（ログイン不要） | 問い合わせのまとまり数（上限なし） | 1,600で 170 µs/投稿（投稿ごとに正規化） | 接続時に1回正規化、100×10×128字まで |
| リモートの投票の更新 | 選択肢数（上限なし） | k=2.04、8,000個で 134 ms | Mapで k≈1.1、取り込みは100個まで |
| 投稿削除時の宛先の重複除去 | 返信・リノートの件数 | findIndex で2乗 | SQLで投稿者を重複なく取り、Mapで線形 |
| Acceptヘッダーの判定（3xx応答時） | ヘッダー長 | k=1.91、32 KB で 42 ms（`[^,]*`に変えても2乗のまま） | メディア型ごとの includes で 34 µs |
| 禁止ワード等の正規表現 | 判定ごと | 毎回 new RegExp | 設定ごとに使い回し（g/y は lastIndex を戻す） |

投票の取り込みは、名前の無い選択肢を選択肢の側だけ除いていたため票数と位置がずれ得たのも同時に直った。回帰テストは17件（時間の上限を置いたものは、変更前の実装で落ちることを確認）。残りの4件のうち3件は続けて測った。投稿の hint 作成（最大1,000人ずつ）で利用者ごとにリアクションの合算・正規化と一覧の連結をやり直していた点は、投稿ごとに1回へ移し、利用者1,000人・投稿2件・リアクション200種類の写しで 99 ms → 0.1 ms。JSON-LD の compact は tag 25〜400件で k=0.88 と線形で、受信の64 KB 上限もあるため、署名検証の意味に触れる処理順の入れ替えは行わない。ストリーミング接続の解除は1.6万接続で1回 79 µs（全接続への通知2回分）で、見送った。管理者が書いた禁止ワード等の正規表現は、利用者の判断で純JSの re2js（MIT・依存なし）で照合するようにした（npm の re2 は NAN 製で Bun では読めない）。`(a+)+$` は JS の RegExp で a 24個 174 ms（2個ごとに約4倍）→ re2js で約2 µs、128 KB の入力でも 10.7 ms と線形。普通のパターンが一致しない 8 KB の投稿は 0.9 → 0.8 µs と変わらない。RE2 に無い構文（後方参照・先読み）は admin/update-meta の入力検証で弾き、既存設定が黙って効かなくなることを防ぐ。

### MFMの閉じない開き記号による2乗の解消（2026-09-25）

frontend は投稿・プロフィール・チャットを表示のたびに mfm-js でパースする。リンクのラベル・`\(`・`\[`・`<center>`・`<plain>`・`<https://` は、開き記号の後を閉じ記号まで読み進めてから後続を確かめる。閉じの無い開き記号が並ぶと開始位置ごとに行末（`\[` と `<center>` は入力末尾）まで読み直し、長さの2乗（k≈2）になっていた。ブラウザと同じ V8 で 8,192字の `[` が 8,784 ms、`\[`+改行が 4,299 ms。リモートの投稿は本文を相手が選べるので、1件で表示中の全クライアントが止まる。

本文ループのある位置から先の結果は、入力・位置・深さ・リンクラベル内かどうかだけで決まり、many は後戻りしない。そこで失敗した試行が通った位置を覚え、後の試行がそこへ来たら読まずに失敗とする（`scanThen`）。リンクの url が文字列になって不成立になる判定も後続の中へ入れ、記憶の対象にした。全構文を測り直して k は 0.8〜1.2、8,192字の `[` は 8.7 ms、普通の文は約600 µs で変化は揺れの範囲内。深さを記憶の鍵に含めないと、入れ子の上限で失敗したラベルが浅い位置でも失敗扱いになる（`<center>` が閉じ損ねた後のインラインコード入りリンク）。差分テスト（元の文法との一致）にこの入力と開き記号の並びを加え、記憶を外す・深さ・入力の区別・成功位置の記憶・url 判定の各変異が落ちることを確かめた。リンクラベル内外の区別を外す変異は、現在の構文では結果が変わらない。

dev サーバーで3,000字（ローカルの上限）の投稿を詳細表示すると、変更前はページ起動とは別に `[` で 1,324〜1,339 ms、`\[`+改行で 542〜617 ms の long task が出ていた。変更後は普通の投稿と同じく 118 ms 以下になった（各3回）。

### frontendの計算量と表示の重さ（2026-09-25）

MFMの2乗を直した後、残りの候補を関数単位で測り、利用者の操作で効くものを直した。dev サーバーに絵文字1万件（100カテゴリ）と下書き1,000件を入れ、同じ手順を変更前・変更後で各2回、Chromium で測った（処理時間は CDP の TaskDuration の差分）。

| 操作 | 原因 | 変更前 | 変更後 |
| --- | --- | --- | --- |
| 投稿ダイアログで96字入力（下書き1,000件、789 KB） | 下書きは返信先ごとに残り上限が無く、1打鍵ごとに全件を JSON で読み書き | 処理 2,642〜3,022 ms | 219〜437 ms（最近の50件、39 KB） |
| 絵文字150件を追加（1件ごとに emojiAdded） | 更新ごとに全絵文字を IndexedDB へ構造化複製（1万件で約12 ms） | 処理 3,322〜3,445 ms | 1,277〜1,499 ms（保存は最後の更新の1秒後に1回） |
| about の絵文字タブを開く | 閉じたカテゴリも v-show で全件描画、カテゴリごとに全件走査 | long task 1,071〜1,164 ms、img 1万個 | 76〜109 ms、開くまで描画しない |
| リアクションピッカーを開く | 最上位フォルダごとに全件走査、不可判定は配列の includes | 208〜307 ms | 92〜119 ms（カテゴリ別の一覧を1回で作り Set で判定） |
| 3,000字の行で入力補完 | 非固定の `/[...]+$/` が各位置から空白まで読み直す | 3,000字で6.3 ms/打鍵、6,000字で25 ms | 末尾から遡る走査 |
| 管理画面の表の重複検査 | セルごとに全セルを走査 | 1,000行で66 ms、3,000行で370 ms/回 | 検証ごとに値の索引を1回作る |

下書きは、本文・CW・添付・アンケートが空になったら消す。localStorage の容量の上限に届くと以後の保存が例外で失敗していたので、上限は速さと保存の失敗の両方に効く。IndexedDB への保存を pagehide で書き出す案は、idb-keyval が DB を開く Promise を待ってから put するため Chromium では確定せず、採らなかった。閉じる直前1秒の変更は保存されないが、古い一覧が古い取得時刻とともに残るので、30秒の鮮度を過ぎていれば起動時に取り直す。チャットと Page のテキストは、描画と URL 抽出で同じ構文木を使うようにした。

見送ったもの: 投稿ごとの noteUpdated 購読（2,000件表示で16 µs/イベント、全件のアンマウントで12 ms）、フォローボタンの main チャンネル（共有接続のため接続は1本）、チャットルームの1通ごとの走査・Paginator の配列複製（件数に上限がある）、アカウント情報の保存（1回0.1 ms未満）、打鍵ごとの MFM プレビュー（パースが線形になり3,000字で約0.6 ms）、ハッシュタグ履歴（増えるのは投稿ごとの新しいタグだけで、5,000種でも読み込み約0.3 ms）。ワードミュートの正規表現は利用者本人の非公開の設定なので、backend の禁止ワードのような RE2 化はしていない。調査の過程で、日付区切りの `day` 指定が時刻まで比べていた（呼び出し元が無いので指定ごと削除）ことと、リストから外したときに見つからないと末尾の要素を消していたことも直した。

### 初回表示（2026-09-25）

本番ビルドを本番モードで起動し、ログイン済みでキャッシュの無い状態からホームのタイムラインに投稿が出るまでを、CDP の回線制限で測った（絵文字1万件・投稿40件、各3回）。localhost のままでは転送が一瞬で差が見えないため、速め（往復40 ms・10 Mbps）と遅め（往復150 ms・1.6 Mbps）の2条件にした。

| 段階 | 速め | 遅め | 転送量 |
| --- | --- | --- | --- |
| 変更前 | 2.35〜2.36 秒 | 12.35〜12.36 秒 | 2,187 KB |
| 応答の圧縮 | 1.25〜1.31 秒 | 4.29〜4.32 秒 | 503 KB |
| 静的な依存の先読み・既定画像の先読み停止 | 1.14〜1.15 秒 | 3.89〜3.94 秒 | 430 KB |
| アイコンCSS・画面の骨組みの並行読み込み | 1.06〜1.14 秒 | 3.63〜3.68 秒 | 430 KB |

- サーバーは応答を一切圧縮していなかった。最大は絵文字一覧で、1万件で 1,170 KB（gzip で 47 KB）。起動処理はこの取得を待つ。API・HTML は Hono の compress（gzip）、静的ファイルは最初の要求で brotli（品質9）/ gzip にして保持する（1言語分の JS・CSS 5.2 MB が 30%）。全言語分をビルド時に作ると 1言語0.4〜0.6秒×30 かかるので要求分だけにした。圧縮の CPU は絵文字一覧で約7 ms、17 KB の API 応答で約0.1 ms。API の JSON には Content-Length を付け、1 KB 未満（ping など）は圧縮しない。
- modulepreload は指定したファイルしか取らず依存を辿らないのに、入口の直下しか指定していなかった。依存が見つかるたびに往復1回ずつ待ち、入口の後に3段あった。静的な依存を奥まで指定した（途中で manifest に無いチャンクがあると収集を打ち切る誤りも直した）。
- 未設定のエラー・お知らせ・404 画像に外部サイトの既定画像（3枚73 KB）を先読みさせていたが、クライアントは未設定なら画像を出さない。使われない読み込みが起動の帯域を取り合っていた。
- アイコンCSS の読み込みを待ってから起動処理を読み始め、絵文字一覧の取得を待ってから画面の骨組み（universal.vue）を読み始めていた。どちらも並行させた。骨組みの読み込みはストアとアカウントの初期化の後に始める。

残る直列は、全 JS の読み込み後に始まる API（i・emojis）と、骨組みの子（お知らせ・ステータスバー・ウィジェット）とタイムライン API の2段で、遅めの回線で約0.8秒。

続けて調べた結果、この残りは直列の待ちではなく転送量で決まっていた（各3回）。
- 絵文字一覧をローダーから同じ URL・credentials で先に取ると、アプリの要求は HTTP キャッシュから 2〜3 ms で返るが、表示は変わらない（遅め 3.65 秒、速め 1.07 秒）。待ちは骨組みの JS の転送へ移るだけ。
- Caddy を前に置いた HTTP/2（運用に近い形）でも、速めは 1.18〜1.25→0.88〜1.13 秒、遅めは 3.66〜3.75→3.39〜3.68 秒でほぼ同じ。遅めは 444 KB÷200 KB/秒≒2.2 秒が転送。
- 骨組みとその子の JS 17本の modulepreload と絵文字一覧の先読みを加える上限見積もりは、遅めで 3.36〜3.61→3.92〜4.06 秒と悪化し、転送量が 444→486 KB に増えた（帯域の取り合いと重複取得）。
- 起動時に読む JS（68チャンク、展開後 787 KB、br 約 244 KB）の内訳に大物は無く、初回表示に要らないもの（json5 30.7 KB、投稿フォーム 31 KB、メディアビューア 19 KB、ドライブ 15 KB、絵文字ピッカー 11 KB、いずれも展開後）を全部外しても br で約 34 KB（遅めで約 0.17 秒）。投稿フォームと絵文字ピッカーは iOS のフォーカスのため同期 import で、json5 を外すと computed を含む8箇所が非同期化する。費用に見合わないため入れていない。

### 長時間利用時のメモリ（2026-09-25）

本番ビルドでホームを開いたまま、投稿の流入・6画面の行き来（SPA のまま）・リアクションピッカーの開閉を1周として12周し、周ごとに GC を2回かけてから CDP の JSHeapUsedSize・Nodes・JSEventListeners を取った（絵文字1万件）。変更前は周ごとに DOM ノード約1,500・リスナー約206・ヒープ約1.2 MB が増え続け、12周でノード 2,171→20,377、ヒープ 9.0→28.0 MB。画面上の要素数は一定だった。操作を分けると、投稿の流入と画面の行き来は1周目以降増えず、ピッカーの開閉だけが1回あたり約1,300ノード・0.87 MB 残していた。

ヒープスナップショットで、切り離された `<section>`（フォルダ）から GC ルートへの最短経路を、WeakMap の値の辺（キーが生きている間だけの保持）を除いて辿ると、モジュールの絵文字一覧 computed の購読者に、ピッカーのテンプレート内で作った `computed(() => ...)` が残り、その closure がピッカーの renderCache（v-once）経由でフォルダの DOM を保持していた。テンプレートで computed を作らず、フォルダの一覧は配列で渡し、リアクション不可の判定は Section が開いたときに自分の computed で作るようにした。変更後は12周でノード 2,530・リスナー 1,083 のまま一定、ヒープは 12.7→14.6 MB で増分が周ごとに 0.47→0.02 MB と縮む（上限のあるキャッシュが埋まる形）。同じ経路の見直しで、入れ子のフォルダにはリアクション不可の判定が渡っておらず、制限付きの絵文字が選べる表示になっていたのも直した。
...
リモートの actor（20人、各50フォロワー、鍵は DB に登録済みで取りに行かない）から HTTP 署名付きの Create(Note) を `/inbox` に送り、全件保存までの CPU（systemd ユニットの CPUUsageNSec）と SQL 本数（pg_stat_statements）を測った。既定の設定は受信ジョブを毎秒32件で止める（`queues.inbox.maximumStartsPerSecond`）ので、計測では上限を外した。

1件あたり CPU は 4.5〜5.5 ms、SQL 23.4本、Redis の往復は fanout の Lua 1回を含む数回。プロファイルに突出した1箇所は無い（上位は socket 書き込み 10.7%、ioredis 15%、SQL 実行 18% で、どれも本数に比例する）。Bun の cpuprofile は待ち時間をアイドル明けの最初のサンプルに乗せるため、1秒ごとのタイマー処理（outbox の振り分け）が 10〜25% に見えた。サンプル間隔の2倍で頭打ちにして集計すると消える。

削ったのは、統計用に相手サーバーの行を読み直していた SELECT（チャートはホスト名で記録する）と、webhook を持たないリモートの投稿者について webhook を探す SELECT。webhook の送り先があるときだけ投稿を pack する。SQL は 23.4→21.4本/件（3回とも同じ）、CPU は揺れ（4.5〜5.7 ms）の中で差が見えない。Redis の自動パイプライン化も交互に比べたが差が無く（4.64〜4.92 対 4.51〜4.98 ms）、入れていない。単一の actor から送ると投稿数の更新が同じ行で直列化し、処理量が 121件/秒で頭打ちになる（現実の負荷では起きない）。
