...
投票の取り込みは、名前の無い選択肢を選択肢の側だけ除いていたため票数と位置がずれ得たのも同時に直った。回帰テストは17件（時間の上限を置いたものは、変更前の実装で落ちることを確認）。残り: 管理者が書いた禁止ワードの正規表現の破滅的バックトラック（RE2 等の線形エンジンを入れるかは依存追加の判断）、JSON-LD の compact を危険な指示の検査・署名確認より前に行う順序、ストリーミング接続の解除が全接続のリスナー配列を走査する点、通知のリアクション集計を利用者ごとに繰り返す点。
