...
残る直列は、全 JS の読み込み後に始まる API（i・emojis）と、骨組みの子（お知らせ・ステータスバー・ウィジェット）とタイムライン API の2段で、遅めの回線で約0.8秒。API を HTML の段階で先に取る案は、取得処理の資格情報の扱いを preload と一致させる必要があり、今回は入れていない。
