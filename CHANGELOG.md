...
- Fix: 仮想化されたタイムラインで複数のユーザーアイコンが同時に追従する問題を修正
...
- Enhance: 高頻度なDBクエリのSQL組み立てを1回だけ行って使い回すように (drizzleは実行のたびにSQL文字列を組み立て直しており、ホームタイムライン1リクエストあたりのCPUの約40%を占めていた。PostgreSQL側は従来どおり無名文のまま=プランは毎回作り直すため、汎用プランによる劣化は起きない)
...
- Enhance: アンテナインポートのDB書き込みを件数比例の逐次処理から1回の一括INSERTへ変更し、不要なDBキュー経路を削除
- Fix: ジョブキューの待機中にValkeyコマンドタイムアウトが繰り返し発生する問題を修正
- Fix: チャート保存処理の競合で集計値が重複または欠落する問題を修正
- Fix: 分散レート制限の時刻・リクエスト識別・設定検証と、IP制限無効時のユーザー単位制限を修正
- Fix: モデレーターが管理者のパスワードやMFAを変更できる問題を修正
...
- Enhance: 高頻度なノート・アンケート・チャンネル・ドライブファイル取得のSQL組み立てを1回だけ行って使い回すように (同時128接続の負荷計測で `/api/notes` の rps +23〜29%、リクエストあたりCPU −23%、p50 −26%)
...
- Fix: DB接続プールの上限がプロセス単位だったため、ワーカーを増やすとPostgreSQLの `max_connections` を超えて接続が頭打ちになる問題を修正 (`database.pool.maximumConnections` を **ホスト全体の予算** を表す `maximumConnectionsPerHost` (既定60) に変更し、DBを使うプロセス数で分割するように。既定構成では従来と同じ1プロセスあたり30接続)
...
- Enhance: 未認証で叩けるRSS取得を強化 (スキーマとURL長の検証、認証情報入りURLの拒否、同一URLへの同時取得の共有、同時実行数の上限、XMLパースの非同期化、取得先の詳細を含まないエラー応答)
...
- Bun v1.3.14 を必須にしました。
  - Node.js / pnpm ベースの実行環境から Bun ベースの実行環境へ移行しました。
  - Docker Image は `oven/bun:1.3.14` 系に更新されています。
...
- Enhance: Bun 1.3.14以降をサポートし、Node.js / pnpm 要件を廃止
- Enhance: Docker Image を `oven/bun:1.3.14` 系に更新
