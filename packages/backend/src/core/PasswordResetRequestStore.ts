...
export async function createPasswordResetRequestInDatabase(
	db: MiDrizzleDatabase,
	data: PasswordResetRequestInsert,
): Promise<void> {
...
export async function fetchPasswordResetRequestByTokenFromDatabase(
	db: MiDrizzleDatabase,
	token: string,
): Promise<PasswordResetRequestRow | null> {
	const [row] = await db.select().from(passwordResetRequest).where(eq(passwordResetRequest.token, token)).limit(1);
...
export async function consumePasswordResetRequestInDatabase(
	db: MiDrizzleDatabase,
	token: string,
	passwordHash: string,
): Promise<ConsumePasswordResetRequestResult> {
