...
export const announcement = pgTable('announcement', {
	...
	icon: varchar({ length: 256, enum: ['info', 'warning', 'error', 'success'] }).default('info').notNull(),
	display: varchar({ length: 256, enum: ['normal', 'banner', 'dialog'] }).default('normal').notNull(),
