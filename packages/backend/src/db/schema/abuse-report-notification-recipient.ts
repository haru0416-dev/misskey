...
export const abuseReportNotificationRecipient = pgTable('abuse_report_notification_recipient', {
	...
	userId: varchar({ length: 32 }).$type<MiUser['id'] | null>().default(null).references(() => user.id, { onDelete: 'cascade' }),
	systemWebhookId: varchar({ length: 32 }).$type<MiSystemWebhook['id'] | null>().default(null).references(() => systemWebhook.id, { onDelete: 'cascade' }),
}, table => [
