...
export async function handleHonoApiUsersListsFavorite(
	...
	if (!exists) {
		throw clientError(400, 'No such user list.', 'NO_SUCH_USER_LIST', '7dbaf3cf-7b42-4b8f-b431-b3919e580dbe');
	}

	if (await userListFavoriteExistsInDatabase(deps.db, me.id, params.listId)) {
		throw clientError(400, 'The list has already been favorited.', 'ALREADY_FAVORITED', '6425bba0-985b-461e-af1b-518070e72081');
	}
...
export async function handleHonoApiUsersListsUnfavorite(
	...
	const exists = await userListExistsByIdAndPublicFromDatabase(deps.db, params.listId);
	if (!exists) {
		throw clientError(400, 'No such user list.', 'NO_SUCH_USER_LIST', 'baedb33e-76b8-4b0c-86a8-9375c0a7b94b');
	}

	const favorite = await fetchUserListFavoriteFromDatabase(deps.db, me.id, params.listId);
	if (favorite == null) {
		throw clientError(400, 'You have not favorited the list.', 'ALREADY_FAVORITED', '835c4b27-463d-4cfa-969b-a9058678d465');
	}
...
export async function handleHonoApiClipsFavorite(
	...
	if (clip == null || (clip.userId !== me.id && !clip.isPublic)) {
		throw clientError(400, 'No such clip.', 'NO_SUCH_CLIP', '4c2aaeae-80d8-4250-9606-26cb1fdb77a5');
	}

	if (await clipFavoriteExistsInDatabase(deps.db, me.id, clip.id)) {
		throw clientError(400, 'The clip has already been favorited.', 'ALREADY_FAVORITED', '92658936-c625-4273-8326-2d790129256e');
	}
...
export async function handleHonoApiClipsUnfavorite(
	...
	const clip = await fetchClipByIdFromDatabase(deps.db, params.clipId);
	if (clip == null) {
		throw clientError(400, 'No such clip.', 'NO_SUCH_CLIP', '2603966e-b865-426c-94a7-af4a01241dc1');
	}

	const favorite = await fetchClipFavoriteFromDatabase(deps.db, me.id, clip.id);
	if (favorite == null) {
		throw clientError(400, 'You have not favorited the clip.', 'NOT_FAVORITED', '90c3a9e8-b321-4dae-bf57-2bf79bbcc187');
	}
...
export async function handleHonoApiChannelsFavorite(
	...
	if (channel == null) {
		throw clientError(400, 'No such channel.', 'NO_SUCH_CHANNEL', '4938f5f3-6167-4c04-9149-6607b7542861');
	}
...
export async function handleHonoApiChannelsUnfavorite(
	...
	const channel = await fetchChannelByIdFromDatabase(deps.db, params.channelId);
	if (channel == null) {
		throw clientError(400, 'No such channel.', 'NO_SUCH_CHANNEL', '353c68dd-131a-476c-aa99-88a345e83668');
	}
...
export async function handleHonoApiPagesLike(
	...
	if (page == null) {
		throw clientError(400, 'No such page.', 'NO_SUCH_PAGE', 'cc98a8a2-0dc3-4123-b198-62c71df18ed3');
	}
	if (page.userId === me.id) {
		throw clientError(400, 'You cannot like your page.', 'YOUR_PAGE', '28800466-e6db-40f2-8fae-bf9e82aa92b8');
	}

	if (await pageLikeExistsInDatabase(deps.db, me.id, page.id)) {
		throw clientError(400, 'The page has already been liked.', 'ALREADY_LIKED', 'd4c1edbe-7da2-4eae-8714-1acfd2d63941');
	}
...
export async function handleHonoApiPagesUnlike(
	...
	const page = await fetchPageByIdFromDatabase(deps.db, params.pageId);
	if (page == null) {
		throw clientError(400, 'No such page.', 'NO_SUCH_PAGE', 'a0d41e20-1993-40bd-890e-f6e560ae648e');
	}

	const like = await fetchPageLikeFromDatabase(deps.db, me.id, page.id);
	if (like == null) {
		throw clientError(400, 'You have not liked that page.', 'NOT_LIKED', 'f5e586b0-ce93-4050-b0e3-7f31af5259ee');
	}
...
export async function handleHonoApiFlashLike(
	...
	if (flash == null) {
		throw clientError(400, 'No such flash.', 'NO_SUCH_FLASH', 'c07c1491-9161-4c5c-9d75-01906f911f73');
	}
	if (flash.userId === me.id) {
		throw clientError(400, 'You cannot like your flash.', 'YOUR_FLASH', '3fd8a0e7-5955-4ba9-85bb-bf3e0c30e13b');
	}

	if (await flashLikeExistsInDatabase(deps.db, me.id, flash.id)) {
		throw clientError(400, 'The flash has already been liked.', 'ALREADY_LIKED', '010065cf-ad43-40df-8067-abff9f4686e3');
	}
...
export async function handleHonoApiFlashUnlike(
	...
	const flash = await fetchFlashByIdFromDatabase(deps.db, params.flashId);
	if (flash == null) {
		throw clientError(400, 'No such flash.', 'NO_SUCH_FLASH', 'afe8424a-a69e-432d-a5f2-2f0740c62410');
	}

	const like = await fetchFlashLikeFromDatabase(deps.db, me.id, flash.id);
	if (like == null) {
		throw clientError(400, 'You have not liked that flash.', 'NOT_LIKED', '755f25a7-9871-4f65-9f34-51eaad9ae0ac');
	}
