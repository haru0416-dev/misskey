 ...
 *
 * list ごとのコマンドを pipeline に積むと、フォロワー 87 人の投稿で 215 コマンドになり、その組み立てだけで
 * notes/create の CPU の 10% を占める (2026-09-03 実測)。スクリプトなら送るのは鍵と上限の一覧だけで済む。
 *
 * outbox の再試行で同じ投稿が二重に入るのは許容し、重複は読み取り側で除く。lrem で重複を防ぐと、見つからない
 * 場合も list を末尾まで走査し、長さ 300 の list で 1 回 4.75µs (lpush の約 10 倍)、スクリプト時間の 55% を
 * 占めて Valkey の単一スレッドを塞ぐ (2026-09-24 実測)。二重になるのは配布後・完了記録前に失敗した
 * 再試行だけで、list の枠を 1 つ使うに留まる。
 * 切り詰めは LLEN で判定するので、list が上限を超えたまま残ることは無い。
 *
