...
export const adminEndpoints = implementEndpoints<ApiShellDependencies>()(adminContracts, {
	...
	'admin/announcements/create': async ({ deps, input, me }) => await handleApiAdminAnnouncementsCreate(deps, me, input),
	...
	'admin/update-proxy-account': async ({ deps, input, me }) => await handleApiAdminUpdateProxyAccount(deps, me, input),
	...
	'admin/get-index-stats': async ({ deps, input }) => await handleApiAdminGetIndexStats(deps, input),
	'admin/get-table-stats': async ({ deps, input }) => await handleApiAdminGetTableStats(deps, input),
	'admin/drive/show-file': async ({ deps, me, input }) => await handleApiAdminDriveShowFile(deps, me, input),
	'admin/update-meta': async ({ deps, me, input }) => {
		await handleApiAdminUpdateMeta(deps, me, input);
	},
});
