...
export const iEndpoints = implementEndpoints<ApiShellDependencies>()(iContracts, {
	...
	'i/2fa/register-key': async ({ deps, errors, input, me }) => await handleApiI2faRegisterKey(deps, me, input, errors),
	...
	'i/registry/get-all': async ({ auth, deps, input, me }) => await handleApiRegistryGetAll(deps, me, auth.token, input),
	...
	'i/update': async ({ auth, deps, errors, input, me }) => await handleApiIUpdate(deps, me, auth.token, input, errors),
	...
	'i/import-antennas': async ({ deps, me, input }) => {
		...
		await handleApiIImportAntennas(deps, me, input, () =>
			assertApiRateLimitForUser(deps, 'i/import-antennas', { duration: HOUR, max: 1 }, me),
		);
	},
