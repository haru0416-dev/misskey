...
export const adminQueueEndpoints = implementEndpoints<ApiShellDependencies>()(adminQueueContracts, {
	...
	'admin/queue/outbox-dead-letters': async ({ deps, input }) => await handleApiAdminQueueOutboxDeadLetters(deps, input),
	...
	'admin/queue/deliver-delayed': async ({ deps }) => await handleApiAdminQueueDeliverDelayed(deps),
	'admin/queue/inbox-delayed': async ({ deps }) => await handleApiAdminQueueInboxDelayed(deps),
	'admin/queue/stats': async ({ deps }) => await handleApiAdminQueueStats(deps),
	'admin/queue/queues': async ({ deps }) => await handleApiAdminQueueQueues(deps),
});
