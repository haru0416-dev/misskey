...
function createSummary(): UrlPreviewSummary {
	return {
...
describe('createUrlPreviewService', () => {
	test('reuses a cached summary without mutating the cached original', async () => {
		...
		try {
			...
			expect((first as UrlPreviewSummary).icon).toContain('https://media.test/preview.webp?');
	...
	test('marks matching URL previews as sensitive', async () => {
		...
		try {
			...
			expect((result as UrlPreviewSummary).sensitive).toBe(true);
		} finally {
			service.dispose();
		}
	});
	...
	test('preserves Summaly sensitive result and ignores invalid or unsafe regular expressions', async () => {
		const summary = { ...createSummary(), sensitive: true } as UrlPreviewSummary;
