...
describe('translateTextForHonoApi', () => {
	test('uses DeepL by default and validates its response', async () => {
		const send = vi.fn(async (_url: SendArgs[0], _args?: SendArgs[1]) => new Response(JSON.stringify({
			translations: [{ detected_source_language: 'JA', text: 'Hello' }],
		})));

		await expect(translateTextForHonoApi({
			meta: meta({ deeplAuthKey: 'secret' }),
			httpRequestService: { send },
		}, 'こんにちは', 'en')).resolves.toEqual({ sourceLang: 'JA', text: 'Hello' });
		expect(send).toHaveBeenCalledWith('https://api-free.deepl.com/v2/translate', expect.objectContaining({
			method: 'POST',
	...
	test('includes the optional LibreTranslate API key', async () => {
		const send = vi.fn(async (_url: SendArgs[0], _args?: SendArgs[1]) => new Response(JSON.stringify({ translatedText: 'Hello' })));
