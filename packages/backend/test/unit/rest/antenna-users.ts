...
describe('antennaUsersIncludes', () => {
	test('matches a specific account without changing existing behavior', () => {
		expect(antennaUsersIncludes(config, ['@Alice@Remote.Example'], { username: 'alice', host: 'remote.example' })).toBe(
			true,
		);
		expect(antennaUsersIncludes(config, ['@alice@remote.example'], { username: 'bob', host: 'remote.example' })).toBe(
			false,
		);
	});

	test('matches every account on a specified server', () => {
		expect(antennaUsersIncludes(config, ['*@REMOTE.EXAMPLE'], { username: 'alice', host: 'remote.example' })).toBe(
			true,
		);
	...
	test('normalizes internationalized domains and supports the local server', () => {
		expect(antennaUsersIncludes(config, ['*@例え.テスト'], { username: 'alice', host: 'xn--r8jz45g.xn--zckzah' })).toBe(
			true,
		);
