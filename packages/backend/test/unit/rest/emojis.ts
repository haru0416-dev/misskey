...
import { fetchEmojiByIdFromDatabase, fetchEmojiByIdOrFailFromDatabase, fetchEmojiByNameAndHostFromDatabaseCached, insertEmojiInDatabase } from '@/core/EmojiStore.js';
...
describe('emoji bulk operations', () => {
	...
	test('updates aliases and metadata atomically while preserving alias order', async () => {
		...
		expect((await fetchEmojiByNameAndHostFromDatabaseCached(runtime.db, first.name, first.host))?.license).toBe('test license');
	...
	test('deletes rows and moderation logs in one transaction', async () => {
		...
		await expect(handleHonoApiAdminEmojiDeleteBulk(deps, { id: genId() } as MiLocalUser, {
			ids: [first.id, second.id],
		})).rejects.toThrow();
		...
		const deleted = publishBroadcastStream.mock.calls.find(call => call[0] === 'emojiDeleted')?.[1].emojis as { id: string }[] | undefined;
		...
		expect(new Set(logs.map(log => (log.info as { emojiId?: string }).emojiId))).toEqual(new Set([first.id, second.id]));
	});
});
