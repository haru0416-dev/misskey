...
describe('ActivityPub object routes', () => {
	...
	test('following page preserves relation order and paginates after batching users', async () => {
		...
		const owner = await createUserWithProfileAndPublickeyInDatabase(runtime.db, {
			user: { id: ownerId, username: `apfollowowner${ownerId}`, usernameLower: `apfollowowner${ownerId}`, followersCount: 11, followingCount: 11 },
			profile: { userId: ownerId, followersVisibility: 'public', followingVisibility: 'public' },
		});
		const base = Date.now() - 20_000;
		const targets = await Promise.all(Array.from({ length: 11 }, async (_, index) => {
			const id = genId();
		...
		for (let index = 0; index < targets.length; index++) {
			followings.push(await createFollowingInDatabase(runtime.db, {
				id: genId(base + index),
				followerId: owner.id,
				followeeId: targets[index]!.id,
			}));
			followers.push(await createFollowingInDatabase(runtime.db, {
