...
describe('Block', () => {
	...
	test('タイムライン(LTL)にブロックされているユーザーの投稿が含まれない', async () => {
		...
		expect(body.some((note) => note.id === aliceNote.id)).toBe(false);
		expect(body.some((note) => note.id === bobNote.id)).toBe(true);
		expect(body.some((note) => note.id === carolNote.id)).toBe(true);
	});
});
