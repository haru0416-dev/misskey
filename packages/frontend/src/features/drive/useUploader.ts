...
export function useUploader(
			...
			menu.push({
				...
				children: [
					...
					action: async () => {
						...
						action: async () => {
							const { dispose } = await os.popupAsyncWithDialog(
								import('@/features/image-editor/components/MkImageEffectorDialog.vue').then((x) => x.default),
								{
									image: item.file,
								},
								{
	...
	async function preprocessForImage(item: UploaderItem): Promise<void> {
		...
		if (needsWatermark && item.watermarkLayers != null) {
			...
			const WatermarkRenderer = await import('@/features/image-editor/watermark/WatermarkRenderer.js').then(
