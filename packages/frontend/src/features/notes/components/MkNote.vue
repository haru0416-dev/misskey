...
import MkNoteSub from '@/features/notes/components/MkNoteSub.vue';
...
import MkInstanceTicker from '@/features/instances/components/MkInstanceTicker.vue';
...
async function loadNoteMenuModule(): Promise<typeof import('@/features/notes/get-note-menu.js') | null> {
	try {
		return await import('@/features/notes/get-note-menu.js');
	} catch (error) {
...
function preloadNotePopupMenu(): void {
	os.preloadPopupMenu();
	void import('@/features/notes/get-note-menu.js').catch(() => {});
}
...
const urls = computed(() =>
	parsed.value
		? extractUrlFromMfm(parsed.value).filter((url) => appearNote.renote?.url !== url && appearNote.renote?.uri !== url)
		: null,
);
...
function checkMute(
	...
	mutedWords: (string | string[])[] | undefined | null,
	checkOnly?: false,
): (string | string[])[] | false | 'sensitiveMute';

function checkMute(
	noteToCheck: Misskey.entities.Note,
	mutedWords: (string | string[])[] | undefined | null,
	checkOnly = false,
): (string | string[])[] | boolean | 'sensitiveMute' {
	if (mutedWords != null) {
...
async function clip(): Promise<void> {
	...
	const currentClipValue = currentClip?.value;
	os.popupMenu(
		await noteMenuModule.getNoteClipMenu({
			note,
...
async function showRenoteMenu() {
	...
	if (isMyRenote) {
		os.popupMenu(
			[...renoteDetailsMenu, getCopyNoteLinkMenu(note, i18n.ts.copyLinkRenote), { type: 'divider' }, getUnrenote()],
			renoteTime.value,
		);
	} else {
		os.popupMenu(
