...
const compiledMutesCache = new WeakMap<(string | string[])[], CompiledMute[]>();

function compileMutes(mutedWords: (string | string[])[]): CompiledMute[] {
	...
	for (const filter of mutedWords) {
		if (Array.isArray(filter)) {
			...
			if (keywords.length > 0) {
				compiled.push({ source: filter, keywords });
			}
			continue;
		}
	...
	if (mutedWords.length > 0) {
		...
		for (const filter of compileMutes(mutedWords)) {
			if ('keywords' in filter) {
				if (filter.keywords.every((keyword) => text.includes(keyword))) {
					matched.push(filter.source);
				}
			} else {
