					...
					<MkFolder :defaultOpen="slotProps.isParentOfTarget">
						...
						<div class="_gaps">
							...
							<template v-if="urlPreviewForm.state.urlPreviewEnabled">
								...
								<SearchMarker :keywords="['sensitive', 'thumbnail', 'url']">
									<MkTextarea v-model="urlPreviewForm.state.urlPreviewSensitiveList" tall>
										<template #label><SearchLabel>{{ i18n.ts._urlPreviewSetting.sensitiveList }}</SearchLabel><span v-if="urlPreviewForm.modifiedStates.urlPreviewSensitiveList" class="_modified">{{ i18n.ts.modified }}</span></template>
										<template #caption>{{ i18n.ts._urlPreviewSetting.sensitiveListDescription }}</template>
									</MkTextarea>
								</SearchMarker>
							</template>
						</div>
