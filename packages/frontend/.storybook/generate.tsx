...
function toStories(component: string): Promise<string> {
	...
	const literal = (
		<literal value={component.slice('src/'.length, -'.vue'.length).replace(/\./g, '/')} />
	) as estree.Literal;
	const identifier = (
		<identifier
			name={base
			...
			properties={[
				(
					<property
						...
						value={
							(<literal value={`${dir}/`.startsWith('src/pages/') ? 'fullscreen' : 'centered'} />) as estree.Literal
						}
						kind={'init' as const}
					/>
				) as estree.Property,
	...
	const program = (
		<program
			body={[
					...
					? [
							(
								<import-declaration
									...
									specifiers={[
										(
											<import-namespace-specifier local={(<identifier name="msw" />) as estree.Identifier} />
					...
					? [
							(
								<import-declaration
									...
									specifiers={[
										(
											<import-namespace-specifier local={(<identifier name="storiesMeta" />) as estree.Identifier} />
										) as estree.ImportNamespaceSpecifier,
									]}
								/>
							) as ImportDeclaration,
															...
															init={
																(
																	<satisfies-expression
																						...
																						(
																							<property
																								...
																								value={
																									(
																										<function-expression
																											params={[(<identifier name="args" />) as estree.Identifier]}
																											body={
																												(
																													<block-statement
																														body={[
																		...
																		reference={
																			(<identifier name={`StoryObj<typeof ${identifier.name}>`} />) as estree.Identifier
																		}
																	/>
																) as estree.Expression
															}
				...
				(
					<export-default-declaration declaration={(<identifier name="meta" />) as estree.Identifier} />
...
(async () => {
	const components = [
		...new Set(
			[
				globSync('src/**/*.stories.impl.ts').map((path) => path.replace(/\.stories\.impl\.ts$/, '.vue')),
				globSync('src/**/*.stories.meta.ts').map((path) => path.replace(/\.stories\.meta\.ts$/, '.vue')),
