...
- テスト、Lintが通っていることを予め確認してください。
  - `bun run test`、`bun run lint`でそれぞれ実施可能です
- `bun run api`を実行してAPIレポートを更新し、差分がある場合はコミットしてください。
...
## Tools
### oxlint / oxfmt
このプロジェクトではコードの静的解析に[oxlint](https://oxc.rs/docs/guide/usage/linter)、整形に[oxfmt](https://oxc.rs/docs/guide/usage/formatter)を導入しています。
CI上でも自動でチェックされ、ルールやフォーマットに則っていないコードがあるとエラーになります。
...
ローカル環境でテストを実施するには、`bun run test`を実行してください。
...
PRを作る際は、`bun run api`コマンドを実行してAPIレポートを生成し、差分がある場合はコミットしてください。
