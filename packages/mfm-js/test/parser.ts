...
hoge`;
			const output = [QUOTE([TEXT('foo')]), QUOTE([TEXT('bar')]), TEXT('hoge')];
			assert.deepStrictEqual(mfm.parse(input), output);
		});
	});
	...
	describe('search', () => {
		...
		test('ブロックの前後にあるテキストが正しく解釈される', () => {
			const input = 'abc\nhoge piyo bebeyo 検索\n123';
			const output = [TEXT('abc'), SEARCH('hoge piyo bebeyo', 'hoge piyo bebeyo 検索'), TEXT('123')];
			assert.deepStrictEqual(mfm.parse(input), output);
		});
	});
	...
	describe('code block', () => {
		...
		test('ブロックの前後にあるテキストが正しく解釈される', () => {
			const input = 'abc\n```\nconst abc = 1;\n```\n123';
			const output = [TEXT('abc'), CODE_BLOCK('const abc = 1;', null), TEXT('123')];
			assert.deepStrictEqual(mfm.parse(input), output);
		});
	...
	describe('mathBlock', () => {
		test('1行の数式ブロックを使用できる', () => {
			const input = '\\[math1\\]';
			const output = [MATH_BLOCK('math1')];
			assert.deepStrictEqual(mfm.parse(input), output);
		});
		test('ブロックの前後にあるテキストが正しく解釈される', () => {
			const input = 'abc\n\\[math1\\]\n123';
			const output = [TEXT('abc'), MATH_BLOCK('math1'), TEXT('123')];
			assert.deepStrictEqual(mfm.parse(input), output);
		});
		test('行末以外に閉じタグがある場合はマッチしない', () => {
			const input = '\\[aaa\\]after';
			const output = [TEXT('\\[aaa\\]after')];
			assert.deepStrictEqual(mfm.parse(input), output);
		});
		test('行頭以外に開始タグがある場合はマッチしない', () => {
			const input = 'before\\[aaa\\]';
			const output = [TEXT('before\\[aaa\\]')];
			assert.deepStrictEqual(mfm.parse(input), output);
		});
	});
	...
	describe('center', () => {
		test('single text', () => {
			...
			const output = [CENTER([TEXT('abc')])];
			assert.deepStrictEqual(mfm.parse(input), output);
		});
		test('multiple text', () => {
			const input = 'before\n<center>\nabc\n123\n\npiyo\n</center>\nafter';
			const output = [TEXT('before'), CENTER([TEXT('abc\n123\n\npiyo')]), TEXT('after')];
			assert.deepStrictEqual(mfm.parse(input), output);
		});
	});
	...
	describe('italic alt 1', () => {
		...
		test('basic 2', () => {
			...
			const output = [TEXT('before '), ITALIC([TEXT('abc')]), TEXT(' after')];
			assert.deepStrictEqual(mfm.parse(input), output);
		});
		...
		test('ignore a italic syntax if the before char is neither a space nor an LF nor [^a-z0-9]i', () => {
			...
			input = 'あいう*abc*えお';
			output = [TEXT('あいう'), ITALIC([TEXT('abc')]), TEXT('えお')];
			assert.deepStrictEqual(mfm.parse(input), output);
		});
	});
...
#️⃣abc`;
	...
	describe('url', () => {
		test('basic', () => {
			...
			const output = [N_URL('https://misskey.io/@ai')];
			assert.deepStrictEqual(mfm.parse(input), output);
		});
		...
		test('with comma', () => {
			...
			const output = [N_URL('https://example.com/foo?bar=a,b')];
			assert.deepStrictEqual(mfm.parse(input), output);
		});

		test('ignore trailing comma', () => {
			const input = 'https://example.com/foo, bar';
			const output = [N_URL('https://example.com/foo'), TEXT(', bar')];
			assert.deepStrictEqual(mfm.parse(input), output);
		});

		test('with brackets', () => {
			const input = 'https://example.com/foo(bar)';
			const output = [N_URL('https://example.com/foo(bar)')];
			assert.deepStrictEqual(mfm.parse(input), output);
		});

		test('ignore parent brackets', () => {
			const input = '(https://example.com/foo)';
			const output = [TEXT('('), N_URL('https://example.com/foo'), TEXT(')')];
			assert.deepStrictEqual(mfm.parse(input), output);
		});

		test('ignore parent brackets (2)', () => {
			const input = '(foo https://example.com/foo)';
			const output = [TEXT('(foo '), N_URL('https://example.com/foo'), TEXT(')')];
			assert.deepStrictEqual(mfm.parse(input), output);
		});

		test('ignore parent brackets with internal brackets', () => {
			const input = '(https://example.com/foo(bar))';
			const output = [TEXT('('), N_URL('https://example.com/foo(bar)'), TEXT(')')];
			assert.deepStrictEqual(mfm.parse(input), output);
		});

		test('ignore parent []', () => {
			const input = 'foo [https://example.com/foo] bar';
			const output = [TEXT('foo ['), N_URL('https://example.com/foo'), TEXT('] bar')];
			assert.deepStrictEqual(mfm.parse(input), output);
		});

		test('ignore non-ascii characters contained url without angle brackets', () => {
			const input = 'https://大石泉すき.example.com';
			const output = [TEXT('https://大石泉すき.example.com')];
			assert.deepStrictEqual(mfm.parse(input), output);
		});

		test('match non-ascii characters contained url with angle brackets', () => {
			const input = '<https://大石泉すき.example.com>';
			const output = [N_URL('https://大石泉すき.example.com', true)];
			assert.deepStrictEqual(mfm.parse(input), output);
		});

		test('prevent xss', () => {
			const input = 'javascript:foo';
			const output = [TEXT('javascript:foo')];
			assert.deepStrictEqual(mfm.parse(input), output);
		});
	});
	...
	describe('link', () => {
		test('basic', () => {
			const input = '[official instance](https://misskey.io/@ai).';
			const output = [LINK(false, 'https://misskey.io/@ai', [TEXT('official instance')]), TEXT('.')];
			assert.deepStrictEqual(mfm.parse(input), output);
		});

		test('silent flag', () => {
			const input = '?[official instance](https://misskey.io/@ai).';
			const output = [LINK(true, 'https://misskey.io/@ai', [TEXT('official instance')]), TEXT('.')];
			assert.deepStrictEqual(mfm.parse(input), output);
		});
		...
		test('prevent xss', () => {
			...
			const output = [TEXT('[click here](javascript:foo)')];
			assert.deepStrictEqual(mfm.parse(input), output);
		});
		...
		describe('cannot nest a mention in a link label', () => {
			test('basic', () => {
				...
				const output = [LINK(false, 'https://example.com', [TEXT('@example')])];
				assert.deepStrictEqual(mfm.parse(input), output);
			});
			test('nested', () => {
				const input = '[@example**@example**](https://example.com)';
				const output = [LINK(false, 'https://example.com', [TEXT('@example'), BOLD([TEXT('@example')])])];
				assert.deepStrictEqual(mfm.parse(input), output);
			});
		});
		...
		test('with brackets', () => {
			const input = '[foo](https://example.com/foo(bar))';
			const output = [LINK(false, 'https://example.com/foo(bar)', [TEXT('foo')])];
			assert.deepStrictEqual(mfm.parse(input), output);
		});

		test('with parent brackets', () => {
			const input = '([foo](https://example.com/foo(bar)))';
			const output = [TEXT('('), LINK(false, 'https://example.com/foo(bar)', [TEXT('foo')]), TEXT(')')];
			assert.deepStrictEqual(mfm.parse(input), output);
		});

		test('with brackets before', () => {
			const input = '[test] foo [bar](https://example.com)';
			const output = [TEXT('[test] foo '), LINK(false, 'https://example.com', [TEXT('bar')])];
			assert.deepStrictEqual(mfm.parse(input), output);
		});

		test('bad url in url part', () => {
			const input = '[test](http://..)';
			const output = [TEXT('[test](http://..)')];
			assert.deepStrictEqual(mfm.parse(input), output);
		});
	});
	...
	describe('fn', () => {
		...
		test('with a string argument', () => {
			...
			const output = [FN('spin', { speed: '1.1s' }, [TEXT('a')])];
			assert.deepStrictEqual(mfm.parse(input), output);
		});

		test('with a string argument 2', () => {
			const input = '$[position.x=-3 a]';
			const output = [FN('position', { x: '-3' }, [TEXT('a')])];
			assert.deepStrictEqual(mfm.parse(input), output);
		});

		test('invalid fn name', () => {
			const input = '$[関数 text]';
			const output = [TEXT('$[関数 text]')];
			assert.deepStrictEqual(mfm.parse(input), output);
		});

		test('nest', () => {
			const input = '$[spin.speed=1.1s $[shake a]]';
			const output = [FN('spin', { speed: '1.1s' }, [FN('shake', {}, [TEXT('a')])])];
			assert.deepStrictEqual(mfm.parse(input), output);
		});
	});
	...
	describe('nesting limit', () => {
		describe('quote', () => {
			test('basic', () => {
				const input = '>>> abc';
				const output = [QUOTE([QUOTE([TEXT('> abc')])])];
		...
		test('big', () => {
			...
			const output = [BOLD([BOLD([TEXT('***abc***')])])];
			assert.deepStrictEqual(mfm.parse(input, { nestLimit: 2 }), output);
		});
		...
		describe('bold', () => {
			test('basic', () => {
				...
				const output = [ITALIC([ITALIC([TEXT('**abc**')])])];
				assert.deepStrictEqual(mfm.parse(input, { nestLimit: 2 }), output);
			});

			test('tag', () => {
				const input = '<i><i><b>abc</b></i></i>';
				const output = [ITALIC([ITALIC([TEXT('<b>abc</b>')])])];
				assert.deepStrictEqual(mfm.parse(input, { nestLimit: 2 }), output);
			});
		});
		...
		test('small', () => {
			...
			const output = [ITALIC([ITALIC([TEXT('<small>abc</small>')])])];
			assert.deepStrictEqual(mfm.parse(input, { nestLimit: 2 }), output);
		});

		test('italic', () => {
			const input = '<b><b><i>abc</i></b></b>';
			const output = [BOLD([BOLD([TEXT('<i>abc</i>')])])];
			assert.deepStrictEqual(mfm.parse(input, { nestLimit: 2 }), output);
		});
		...
		describe('strike', () => {
			...
			test('tag', () => {
				...
				const output = [BOLD([BOLD([TEXT('<s>abc</s>')])])];
				assert.deepStrictEqual(mfm.parse(input, { nestLimit: 2 }), output);
			});
		});
		...
		describe('hashtag', () => {
			test('basic', () => {
				let input, output;
				input = '<b>#abc(xyz)</b>';
				output = [BOLD([HASHTAG('abc(xyz)')])];
				assert.deepStrictEqual(mfm.parse(input, { nestLimit: 2 }), output);
				input = '<b>#abc(x(y)z)</b>';
				output = [BOLD([HASHTAG('abc'), TEXT('(x(y)z)')])];
				assert.deepStrictEqual(mfm.parse(input, { nestLimit: 2 }), output);
			});
		...
		test('url', () => {
			...
			output = [BOLD([N_URL('https://example.com/abc(xyz)')])];
			assert.deepStrictEqual(mfm.parse(input, { nestLimit: 2 }), output);
			input = '<b>https://example.com/abc(x(y)z)</b>';
			output = [BOLD([N_URL('https://example.com/abc'), TEXT('(x(y)z)')])];
			assert.deepStrictEqual(mfm.parse(input, { nestLimit: 2 }), output);
		});

		test('fn', () => {
			const input = '<b><b>$[a b]</b></b>';
			const output = [BOLD([BOLD([TEXT('$[a b]')])])];
			assert.deepStrictEqual(mfm.parse(input, { nestLimit: 2 }), output);
		});
	});
