...
export class Interpreter {
	...
	@autobind
	private async _fn(fn: VFn, args: Value[], callStack: CallStack, pos?: Ast.Pos): Promise<Value> {
		if (fn.native) {
	...
	@autobind
	private _evalClauseSync(node: Ast.Statement | Ast.Expression, scope: Scope, callStack: CallStack): Value | Control {
	...
	@autobind
	private _evalSync(node: Ast.Node, scope: Scope, callStack: CallStack): Value | Control {
	...
	@autobind
	private async _run(program: Ast.Node[], scope: Scope, callStack: CallStack): Promise<Value | Control> {
	...
	@autobind
	private _runSync(program: Ast.Node[], scope: Scope, callStack: CallStack): Value | Control {
		this.log('block:enter', { scope: scope.name });
	...
	@autobind
	private async getReference(dest: Ast.Expression, scope: Scope, callStack: CallStack): Promise<Reference | Control> {
		switch (dest.type) {
			case 'identifier': {
	...
	@autobind
	private getReferenceSync(dest: Ast.Expression, scope: Scope, callStack: CallStack): Reference | Control {
		switch (dest.type) {
			case 'identifier': {
				return Reference.variable(dest.name, scope);
			}
