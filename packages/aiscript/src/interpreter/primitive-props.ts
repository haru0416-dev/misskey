...
const PRIMITIVE_PROPS = {
	...
	str: {
		...
		to_unicode_arr: (target: VStr): VFn =>
			FN_NATIVE((_, _opts) => {
				return ARR([...target.value].map((s) => STR(s)));
			}),

		to_unicode_codepoint_arr: (target: VStr): VFn =>
			FN_NATIVE((_, _opts) => {
				return ARR(
					[...target.value].map((s) => {
		...
		starts_with: (target: VStr): VFn =>
			FN_NATIVE(([prefix, start_index], _opts) => {
				assertString(prefix);
				if (!prefix.value) {
					return TRUE;
				}
		...
		ends_with: (target: VStr): VFn =>
			FN_NATIVE(([suffix, end_index], _opts) => {
				assertString(suffix);
				if (!suffix.value) {
					return TRUE;
				}
		...
		pad_start: (target: VStr): VFn =>
			FN_NATIVE(([width, pad], _) => {
				assertNumber(width);
				const s = pad ? (assertString(pad), pad.value) : ' ';
				if (width.value > MAX_NATIVE_VALUE_SIZE) {
					throw new AiScriptRuntimeError(`str.pad_start width must not exceed ${MAX_NATIVE_VALUE_SIZE}`);
				}

				return STR(target.value.padStart(width.value, s));
			}),

		pad_end: (target: VStr): VFn =>
			FN_NATIVE(([width, pad], _) => {
				assertNumber(width);
				const s = pad ? (assertString(pad), pad.value) : ' ';
				if (width.value > MAX_NATIVE_VALUE_SIZE) {
					throw new AiScriptRuntimeError(`str.pad_end width must not exceed ${MAX_NATIVE_VALUE_SIZE}`);
				}
	...
	arr: {
		...
		reduce: (target: VArr): VFn =>
			FN_NATIVE(
				async ([fn, initialValue], opts) => {
					...
					if (!withInitialValue && target.value.length === 0) {
						throw new AiScriptRuntimeError('Reduce of empty array without initial value');
					}
		...
		index_of: (target: VArr): VFn =>
			FN_NATIVE(([val, fromI], _opts) => {
				expectAny(val);
				let start = 0;
				if (fromI) {
					assertNumber(fromI);
		...
		fill: (target: VArr): VFn =>
			FN_NATIVE(([val, st, ed], opts) => {
		...
		repeat: (target: VArr): VFn =>
			FN_NATIVE(([times], opts) => {
				assertNumber(times);
				if (times.value < 0) {
					throw new AiScriptRuntimeError('arr.repeat expected non-negative number, got negative');
				}
				if (!Number.isInteger(times.value)) {
					throw new AiScriptRuntimeError('arr.repeat expected integer, got non-integer');
				}
				if (target.value.length === 0) {
					return ARR([]);
				}
				if (target.value.length * times.value > MAX_NATIVE_VALUE_SIZE) {
					throw new AiScriptRuntimeError(`arr.repeat size must not exceed ${MAX_NATIVE_VALUE_SIZE}`);
				}
				return ARR(Array(times.value).fill(target.value).flat());
			}),

		splice: (target: VArr): VFn =>
			FN_NATIVE(([idx, rc, vs], opts) => {
				assertNumber(idx);
				const index =
					idx.value < -target.value.length
						? 0
						: idx.value < 0
							? target.value.length + idx.value
							: idx.value >= target.value.length
								? target.value.length
								: idx.value;

				const remove_count = rc != null ? (assertNumber(rc), rc.value) : target.value.length - index;
		...
		flat: (target: VArr): VFn =>
			FN_NATIVE(([depth], opts) => {
				depth = depth ?? NUM(1);
				assertNumber(depth);
				if (!Number.isInteger(depth.value)) {
					throw new AiScriptRuntimeError('arr.flat expected integer, got non-integer');
				}
				if (depth.value < 0) {
					throw new AiScriptRuntimeError('arr.flat expected non-negative number, got negative');
				}
				if (depth.value > MAX_ARRAY_FLAT_DEPTH) {
					throw new AiScriptRuntimeError(`arr.flat depth must not exceed ${MAX_ARRAY_FLAT_DEPTH}`);
				}
				const flat = (arr: Value[], depth: number, result: Value[]) => {
					if (depth === 0) {
						if (result.length + arr.length > MAX_NATIVE_VALUE_SIZE) {
							throw new AiScriptRuntimeError(`arr.flat size must not exceed ${MAX_NATIVE_VALUE_SIZE}`);
						}
						result.push(...arr);
						return;
					}
					for (const v of arr) {
						if (isArray(v)) {
							...
							if (result.length >= MAX_NATIVE_VALUE_SIZE) {
								throw new AiScriptRuntimeError(`arr.flat size must not exceed ${MAX_NATIVE_VALUE_SIZE}`);
							}
							result.push(v);
						}
		...
		insert: (target: VArr): VFn =>
			FN_NATIVE(([index, item], opts) => {
				assertNumber(index);
				expectAny(item);
		...
		remove: (target: VArr): VFn =>
			FN_NATIVE(([index], opts) => {
				assertNumber(index);
		...
		at: (target: VArr): VFn =>
			FN_NATIVE(([index, otherwise], opts) => {
				assertNumber(index);
