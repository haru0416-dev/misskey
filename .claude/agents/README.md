...
- **この `.claude/agents/` の 2 つ**: backend endpoint / Vue SFC の **Misskey 固有・機械的チェック** (endpoint-list 登録漏れ・misskey-js 再生成漏れ・ja-JP.yml 限定・SPDX 形式・コンポーネントカタログ 併設 等)。別コンテキストで差分を機械走査する価値がある領域に限定する
